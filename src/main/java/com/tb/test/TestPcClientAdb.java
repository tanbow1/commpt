...
import com.tb.test.utils.FileHelper;
import com.tb.test.utils.MyUtil;
...
/**
 * Created by Tanbo on 2017/8/24.
 */
public class TestPcClientAdb {
    public static void main(String[] args) throws InterruptedIOException {
        try {
            // adb 指令
            Runtime.getRuntime().exec(
                    "sh -c adb shell am broadcast -a NotifyServiceStop");
            Thread.sleep(3000);
            Runtime.getRuntime().exec("sh -c adb forward tcp:12580 tcp:10086"); // 端口转换
            Thread.sleep(3000);
            Runtime.getRuntime().exec(
                    "sh -c adb shell am broadcast -a NotifyServiceStart");
            Thread.sleep(3000);
        } catch (Exception e) {
            e.printStackTrace();
                ...
                if (strWord.equals("1")) {
                    ...
                    System.out
                            .println("service ready receice data:UPDATE_CONTACTS:"
                                    + strFormsocket);
                    byte[] filebytes = FileHelper.readFile("test.png");
